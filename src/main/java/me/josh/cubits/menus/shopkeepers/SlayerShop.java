...
public class SlayerShop extends CubitGui {
    private static final int INVENTORY_SIZE = 54;
    private static String INVENTORY_NAME; // = "Slayer Shop";

    private SlayerShop(){

    }

    public static void OpenSlayerStore(Main plugin, Player p, String shopName, ItemBase item1, ItemBase item2, ItemBase item3, int favor) {
        // Set Inventory Name
        ...
        // Create Items
        if(SetDailyShopStock.slayer_store_Item1 != null){
            gui.addItem(getShopItem(plugin, item1));
        }

        if(SetDailyShopStock.slayer_store_Item2 != null){
            gui.addItem(getShopItem(plugin, item2));
        }

        if(SetDailyShopStock.slayer_store_Item3 != null){
