...
public class SlayerUpgradeShop extends CubitGui {
    ...
    public static void OpenSlayerUpgradeStore(Main plugin, Player p) {
       // Define Variables
        ...
        if(!playerProfile.getUpgrades().get(PlayerUpgrades.SLAYER_UPGRADE_SPIDER_SQUASHER)){
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Cave Spiders count double towards Spider slayer tasks.", "Cost: 20 Slayer Tokens" ));
        }else{
        ...
        treats_meta.setCustomModelData(10101);
        treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Skip your current slayer task.", "Cost: 10 Slayer Tokens" ));
        ...
        treats_meta.setCustomModelData(10101);
        treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Extend your current slayer task.", "Cost: 5 Slayer Tokens" ));
        ...
        if(!playerProfile.getUpgrades().get(PlayerUpgrades.SLAYER_UPGRADE_DRAGON)){
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Adds an Ender Dragon slayer task.", "Cost: 30 Slayer Tokens" ));
        }else{
        ...
        if(!playerProfile.getUpgrades().get(PlayerUpgrades.SLAYER_UPGRADE_WITHER)){
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Adds a Wither slayer task.", "Cost: 30 Slayer Tokens" ));
        }else{
        ...
        if(!playerProfile.getUpgrades().get(PlayerUpgrades.SLAYER_UPGRADE_WARDEN)){
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Adds a Warden slayer task.", "Cost: 30 Slayer Tokens" ));
        }else{
        ...
        if(!playerProfile.getUpgrades().get(PlayerUpgrades.SLAYER_UPGRADE_BOSSRUSH)){
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Adds a Boss Rush slayer task.", "Includes Ender Dragon, Wither, Elder Guardian, Warden, and Invoker.", "Enables various new item drops and enhanced Slayer Point rewards.", "Cost: 50 Slayer Tokens" ));
        }else{
        ...
        treats_meta.setCustomModelData(10101);
        if(!playerProfile.getUpgrades().get(PlayerUpgrades.SLAYER_UPGRADE_POINTBOOST)){
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Increases the amount of slayer points earned from tasks.", "Cost: 50 Slayer Tokens" ));
        }else{
        ...
        if (playerProfile.getSlayerBlock() == null) {
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Blocks your current slayer task from being assigned, and skips it.", "Only one slayer task can be blocked at once.", "Cost: 50 Slayer Tokens", "Blocked Task - None" ));
        }else{
            treats_meta.setLore(Arrays.asList(ChatColor.WHITE + "Blocks your current slayer task from being assigned, and skips it.", "Only one slayer task can be blocked at once.", "Cost: 50 Slayer Tokens", "Blocked Task - " + playerProfile.getSlayerBlock().toString() ));
        }
        ...
        gui.setItem(11, DRAGON_SLAYER);
        gui.setItem(12, WITHER_SLAYER);
        gui.setItem(13, WARDEN_SLAYER);
        gui.setItem(14, BOSS_RUSH);
        gui.setItem(15, POINT_BOOST);
        gui.setItem(16, EXTEND_TASK);
        gui.setItem(19, BLOCK_SLOT);
        gui.setItem(20, BLOCK_SLOT_CLEAR);
        gui.setItem(21, SKIP_TASK);
        gui.setItem(22, SLAYERTASK);
