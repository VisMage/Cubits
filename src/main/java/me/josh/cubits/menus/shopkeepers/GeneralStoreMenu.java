...
public class GeneralStoreMenu extends CubitGui {
    private static final int INVENTORY_SIZE = 54;
    private static String INVENTORY_NAME; // = "Treat Shop";

    private GeneralStoreMenu(){

    }

    public static void OpenTreatStore(Main plugin, Player p, String shopName, ItemBase item1, ItemBase item2, ItemBase item3, ItemBase item4, ItemBase item5, int favor) {
        // Set Inventory Name
        ...
        loreFavor.add("Unlock slots at 50 and 100 favor.");
        ...
        if(SetDailyShopStock.general_store_Item4 != null){
            if(favor >= 50){
                gui.addItem(getShopItem(plugin, item4));
            }
        }

        if(SetDailyShopStock.general_store_Item5 != null){
            if(favor >= 100){
                gui.addItem(getShopItem(plugin, item5));
            }
        }
