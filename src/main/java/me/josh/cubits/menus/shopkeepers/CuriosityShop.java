...
public class CuriosityShop extends CubitGui {
    private static final int INVENTORY_SIZE = 54;
    private static String INVENTORY_NAME; // = "Treat Shop";

    private CuriosityShop(){

    }

    public static void OpenCuriosityShop(Main plugin, Player p, String shopName, ItemBase item1, ItemBase item2, ItemBase item3, ItemBase item4, ItemBase item5, int favor) {
        // Set Inventory Name
        ...
        // Create Items
        if(SetDailyShopStock.curiosity_merchant_Item1 != null){
            gui.addItem(getShopItem(plugin, item1));
        }

        if(SetDailyShopStock.curiosity_merchant_Item2 != null){
            gui.addItem(getShopItem(plugin, item2));
        }

        if(SetDailyShopStock.curiosity_merchant_Item3 != null){
