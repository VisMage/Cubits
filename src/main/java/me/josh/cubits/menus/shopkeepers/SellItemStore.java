...
public class SellItemStore extends CubitGui {
    private static final int INVENTORY_SIZE = 54;
    private static final String INVENTORY_NAME = "Sell Shop";

    private SellItemStore() {}

    public static void OpenSellItemStore(Main plugin, Player p) {
        // Set Variables
        ...
        // Add sellable items
        if(SetDailyShopStock.general_store_Item1 != null){
            gui.addItem(getSellableItemStack(SetDailyShopStock.sell_Item1, playerProfile, plugin));
        }

        if(SetDailyShopStock.general_store_Item2 != null){
            gui.addItem(getSellableItemStack(SetDailyShopStock.sell_Item2, playerProfile, plugin));
        }

        if(SetDailyShopStock.general_store_Item3 != null){
            gui.addItem(getSellableItemStack(SetDailyShopStock.sell_Item3, playerProfile, plugin));
        }
